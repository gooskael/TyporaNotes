...
## 1.1 CSS字体：px rpx em rem vw vh vmin vmax
...
## 1.3 `vertical-align`怎么使用，配合`text-align`？
...
## 1.4 Flex布局`justify-content`和`align-items`区别
...
## 1.5 定位布局position：static relative absolute fixed sticky
...
## 1.8 inline、block、inline-block、inline-table
...
## 1.10 background 和 background-color
...
`::after`：在组件的最后添加一个伪元素。`::before`：在组件的最前添加一个伪元素，一般配合`content`使用。此外 `:after` 和 `::after` 一致， `:before` 和 `::before` 一致。参考：[::after(:after)](https://developer.mozilla.org/zh-CN/docs/Web/CSS/::after)。
