...
[微信小程序 canvas画布渲染网络图片](https://blog.csdn.net/smile0315/article/details/102708891)

[微信小程序--图片本地缓存与canvas绘制](https://www.jianshu.com/p/407254e1fe22?utm_campaign=maleskine&utm_content=note&utm_medium=seo_notes&utm_source=recommendation)：1.使用`wx.getImageInfo` 2.使用下载缓存到本地，只要小程序不删除，该文件就不会清空

1.canvas生成海报图片长宽比使用`wx.getImageInfo`，再通过计算即可得到图片的比例，不使图片变形。

2.画的时候清晰度要高，展示的时候再按需改变大小。

3.使用`canvas.drawImage(imagePath, x, y, widthImage, heightImage);`的时候，`imagePath`可以是本地图片；要是是网络图片，需要使用`wx.getImageInfo()`加载到图片。

> ```js
> wx.getImageInfo({
> 	src: imageUrlFromNet,
> 	success: function (res) {
> 		canvas.drawImage(res.path, x, y, widthImage, heightImage);
> 	},
> 	fail: function (err) {
> 		console.log(err);
> 	}
...
问题出现是因为，在`wx:for`对列表遍历进行生成元素的时候，希望得到列表中的参数，以便进行进一步操作。

这时候需要对在元素中设置`data-**="dataNeeded"`，然后就可以在`bindtap`绑定的事件中得到返回参数。比如下面图中的`e.currentTarget.dataset`里面就存放着已经传过来的`index`。
