...
#### 3.4 `getUserInfo`获取微信用户信息的接口

- [x] 要求不能在用户未察觉的情况下调用接口，一般使用按钮绑定。
...
#### 4.2 网页传递参数onLoad及onShow中options
...
Page({
  navTO(e) {
    var arr = ['15', '4', '3'];
    wx.navigateTo({
      url: '/pages/onLoadPage/onLoadPage?arr=' + arr,
    })
  }
})
...
Page({
  onLoad(options) {
    // options.arr自动变成字符串，并且还有`,` => "15,4,3"
...
​	2. 在onLoad和onShow中调用options方式不同，onShow要添加`this.`。
...
参考官方[自定义组件](https://developers.weixin.qq.com/miniprogram/dev/framework/custom-component/wxml-wxss.html)
...
[Box-shadow](https://blog.csdn.net/weixin_38747509/article/details/84937447)
