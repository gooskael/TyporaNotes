...
1. 在`src`目录底下新建`store`，在`store`中添加相应的管理文件`index.js`。
...
2. 在`main.js`中引入。在 `main.js`中引入文件的时候，会自动索引`index.js`：`import store from './store'` => `import store from './store/index.js'`。
